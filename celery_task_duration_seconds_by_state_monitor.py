    ...
    def setup_metrics(self):
        ...
        for metric in CELERY_TASK_DURATION_SECONDS_BY_STATE.collect():
            for sample in metric.samples:
                CELERY_TASK_DURATION_SECONDS_BY_STATE.labels(**sample[1]).set(0.0)
...
class CeleryTaskDurationSecondsByStateMonitorThread(
    ...
    def convert_data_to_prometheus(self, data):
        ...
        # from which we extract the task's state.  Also, we only want to know about tasks which
        # are languishing for more than 10 seconds.  Note that going below 5 seconds will bog down the
        # grafana panel for any timerange of more than 1 hour.
